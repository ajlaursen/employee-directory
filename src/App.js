  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const natsArray = Array.from(new Set(sortedArray.map(x => x.nat))).sort();
...
// need help being able to use the drop down menu you filter by country
  ...
  return (
    <div className="App">
      ...
      <select className="form-select" aria-label="Default select example" onChange={handleListChange}>
